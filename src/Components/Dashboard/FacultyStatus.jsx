...
const FacultyStatus = ({ facultyList = [] }) => {
  return (
    <div className="w-[340px] bg-white rounded-[10px] border border-[#CACACA] shadow-[0_2px_8px_rgba(0,0,0,0.08)] overflow-hidden">
      <div className="flex items-center justify-between px-4 py-2 border-b border-[#D9D9D9]">
        <div className="flex items-center gap-2">
          <h3 className="text-[13px] font-semibold text-[#265768]">Faculty</h3>
        ...
        {facultyList.map((member, index) => (
          ...
          >
            <div className="flex items-center gap-3 min-w-0 flex-1">
              <div className="w-10 h-10 rounded-full overflow-hidden bg-[#F3F4F6] flex-shrink-0 border border-[#E5E7EB] flex items-center justify-center text-[12px] text-[#4B5563]">
                {member.name?.charAt(0)?.toUpperCase() || "F"}
              </div>
              <div className="text-[13px] text-[#6B7280] font-normal truncate">
                {member.name}
              </div>
            </div>

            {/* Status column HATA DIYA */}
          </div>
        ))}
      </div>
    </div>
...
//       <div className="flex items-center justify-between px-4 py-2 border-b border-[#D9D9D9]">
//         <h3 className="text-[13px] font-semibold text-[#265768]">Faculty List</h3>
//       </div>

//       <div className="px-3 py-0.1">
//         {facultyList?.map((member) => (
//           <div key={member._id} className="py-2.5 border-b border-[#F3F4F6] text-[13px] text-[#6B7280]">
//             {member.name}
//           </div>
//         ))}
//       </div>
